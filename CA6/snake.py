...
class Snake:
    ...
    def __init__(self, color, pos, file_name=None):
        ...
        self.snake_number = Snake.snake_counter
    ...
    def get_optimal_policy(self, state):
        ...
        # Add penalties for actions that move the snake towards the border
        border_penalty = 10000

        for action in range(4):
            if action == 0 and x - 1 < 1:  # Left
                q_values[action] -= border_penalty
            elif action == 1 and x + 1 >= ROWS - 1:  # Right
                q_values[action] -= border_penalty
            elif action == 2 and y - 1 < 1:  # Up
                q_values[action] -= border_penalty
            elif action == 3 and y + 1 >= ROWS - 1:  # Down
                q_values[action] -= border_penalty
        self_collision_penalty = 10000
        # Check for potential self-collision and add penalties
        for action in range(4):
            if action == 0:  # Left
                next_pos = (x - 1, y)
            elif action == 1:  # Right
                ...
                next_pos = (x, y + 1)
            if next_pos in list(map(lambda z: z.pos, self.body)):
                q_values[action] -= self_collision_penalty

        # Calculate distances to the snack for each action
        distance_rewards = np.zeros(4)
        if x > 0:
            distance_rewards[0] = -abs(snack_x - (x - 1)) - abs(snack_y - y)  # Left
        if x < ROWS - 1:
            distance_rewards[1] = -abs(snack_x - (x + 1)) - abs(snack_y - y)  # Right
        if y > 0:
            distance_rewards[2] = -abs(snack_x - x) - abs(snack_y - (y - 1))  # Up
        if y < ROWS - 1:
            distance_rewards[3] = -abs(snack_x - x) - abs(snack_y - (y + 1))  # Down

        # Add the distance rewards to the Q-values
        q_values += distance_rewards
    ...
    def reset(self, random_pos=False):
        if random_pos:
            pos = (random.randint(2, ROWS - 2), random.randint(2, ROWS - 2))
